...
def process_torch_file(file_path, output_dir):
    ...
    try:
        ...
        for cur_scene in data:
            ...
            cur_info_dict = {"scene_name": scene_name, "frames": []}
                ...
                try:
                    ...
                    frame_info = {
                        "image_path": os.path.join(
                            seq_images_dir, f"{img_idx:05d}.png"
                        ),
                    ...
                    logging.error(
                        f"Error processing image {img_idx} in {file_path}: {str(e)}"
                    )
                    continue
...
def generate_full_list(base_path, output_dir):
    ...
    json_files = [
        os.path.join(base_path, f) for f in os.listdir(base_path) if f.endswith(".json")
    ]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output_dir", type=str, default="/share/phoenix/nfs06/S9/hj453/DATA/re10k/"
    )
    parser.add_argument(
        "--base_path", type=str, default="/share/phoenix/nfs06/S9/hj453/DATA/re10k_raw/"
    )
