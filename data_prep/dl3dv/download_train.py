...
def load_test_scene_hashes(test_view_indices_paths: list[str]) -> set[str]:
    """Load scene hashes from test view indices JSON files.

    Args:
        test_view_indices_paths: List of paths to test view indices JSON files
            (e.g., assets/dl3dv_6v.json). Keys are "batch/hash" strings.

    Returns:
        Set of scene hashes that belong to the test split.
    """
    test_hashes_to_not_download = set()
    for path in test_view_indices_paths:
        with open(path, "r") as f:
            view_indices = json.load(f)
        for key in view_indices:
            # Keys are in format "batch/hash" e.g. "2K/032dee9..."
            parts = key.split("/")
            if len(parts) == 2:
                test_hashes_to_not_download.add(parts[1])
    return test_hashes_to_not_download


def get_batch_scenes(
    batch: str,
    csv_path: str = None,
    test_hashes_to_not_download: set[str] | None = None,
) -> list[str]:
    """Get list of scene hashes for a given batch, excluding test scenes.

    Args:
        batch: Batch name (e.g., "2K", "3K")
        csv_path: Optional path to DL3DV-valid.csv
        test_hashes_to_not_download: Optional set of scene hashes to exclude from download (test split)

    Returns:
        List of scene hash names (with test scenes filtered out)
    """
    ...
    all_hashes = batch_df["hash"].tolist()

    if test_hashes_to_not_download:
        before_count = len(all_hashes)
        all_hashes = [h for h in all_hashes if h not in test_hashes_to_not_download]
        excluded = before_count - len(all_hashes)
        if excluded > 0:
            print(f"Excluded {excluded} test scenes from batch {batch}")
...
def download_train_dataset(args: argparse.Namespace) -> bool:
    ...
    # Load test scene hashes to exclude from training download and .txt
    test_hashes_to_not_download = None
    if args.test_view_indices_paths:
        test_hashes_to_not_download = load_test_scene_hashes(
            args.test_view_indices_paths
        )
        print(
            f"Loaded {len(test_hashes_to_not_download)} test scene hashes to exclude from training download"
        )

    # Get scene list for this batch (excluding test scenes)
    scene_hashes = get_batch_scenes(batch, args.csv_path, test_hashes_to_not_download)
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=None,
        help="Paths to test view indices JSON files (e.g., assets/dl3dv_2v.json assets/dl3dv_6v.json). "
        "Scenes present in these files will be excluded from the download and from the .txt with training sequences.",
    )
    parser.add_argument(
        "--max_scenes",
        type=int,
