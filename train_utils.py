...
@torch.no_grad()
def run_qualitative_visualizations(
    writer,
    model,
    device,
    iter_idx,
    ...
    """Run all qualitative vis on train, validation and fixed validation batch and log to tensorboard."""
    # 1. Fixed test batch visualization
    # Ground truth is the images from the inputs (first element of the tuple)
    fixed_eval_gt = fixed_eval_inputs[0]
    _vis_and_log(
        writer,
        model,
        fixed_eval_inputs,
    ...
    images, rays, _, cam_token, _, num_cond_views = train_batch
    train_vis_inputs = (images, rays, cam_token, num_cond_views)
    _vis_and_log(
        writer,
        model,
        train_vis_inputs,
        images,
        iter_idx,
        vis_name="train_random",
    )

    # 3. Random test batch visualization
    test_batch, test_data_iter, test_epoch_idx = get_next_batch(
        test_data_iter, test_dataset, test_epoch_idx, device
    )
    images_test, rays_test, _, cam_token_test, _, num_cond_views_test = test_batch
    random_eval_inputs = (
        images_test,
        rays_test,
        cam_token_test,
    ...
    _vis_and_log(
        ...
        images_test,
        iter_idx,
        vis_name="eval_random",
    )

    print("Finished writing visualization")
    if misc.is_main_process():
        writer.flush()

    # Return updated iterator, possibly with new poch index
    return test_data_iter, test_epoch_idx


@torch.no_grad()
def _vis_and_log(writer, model, cond_inputs, cond_gt, iter_idx, vis_name):
    """Run single qualitative visualization."""
    cond_image_inputs, rays, cam_token, num_cond_views = cond_inputs
    ...
    cond_psnr = _compute_psnr_tgt(pred_image, cond_gt, num_cond_views[0])

    pred_image = pred_image.cpu()
    cond_gt = cond_gt.cpu()
    vis_grid = torch.cat([pred_image, cond_gt], dim=3)
    grid = _image_tensor_to_grid_numpy(vis_grid)

    if misc.is_main_process():
        writer.add_image(
            f"vis/{vis_name}", grid, global_step=iter_idx, dataformats="HWC"
        )
        writer.add_scalar(f"vis/psnr_{vis_name}", cond_psnr, global_step=iter_idx)


def _image_tensor_to_grid_numpy(batch):
    """Convert batch of images to numpy grid for tensorboard."""
    num_views = batch.shape[1]
    grid = einops.rearrange(batch, "b v c h w -> (b v) c h w")
    grid = torchvision.utils.make_grid(grid[:64], nrow=num_views)
    bigger_dimension = max(grid.shape[1], grid.shape[2])
    resize_factor = 1024 / bigger_dimension
...
def _compute_psnr_tgt(pred_image, cond_gt, num_cond):
    """Compute PSNR on target views (excluding conditioning views)."""
    return (
        -10
        * torch.log10(
            torch.mean(
                (pred_image[:, num_cond:, :, :, :] - cond_gt[:, num_cond:, :, :, :])
                ** 2,
                dim=(2, 3, 4),
            )
        ).mean()
...
def run_quantitative_evaluation(
    ...
    for (
        ...
        cfg.test_data.normalization_mode,
    ):
        print(
        ...
        quant_test_dataset = dataset_factory.available_datasets[dataset_name](
            ...
            normalization_mode=NormalizationMode(test_normalization_mode),
        )
