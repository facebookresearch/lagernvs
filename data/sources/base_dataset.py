...
class BaseDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        if self.normalization_mode == NormalizationMode.NONE:
            print("Warning, not normalizing the scene scale")
    ...
    def get_depth_paths_for_seq(self, seq_name):
        """To be implemented by child classes"""
        raise NotImplementedError
    ...
    def get_idx_from_seq_name(self, seq_name):
        try:
            return self.seq_name_to_idx[seq_name]
        except KeyError:
            print(f"Sequence {seq_name} not found in dataset")
            return None


    def pad_images_and_timestamps_and_concat_poses(
        self,
        images_data,
        c2w_poses_data,
    ...
    def get_video_data(
        ...
        elif self.video_path_type == "test_path":
            traj_c2w_poses = c2w_poses[num_cond_views:]
            traj_fxfycxcy = intrinsics_fxfycxcy_px[num_cond_views:]
        ...
        images, c2w_poses, intrinsics_fxfycxcy_px, new_timesteps = (
            self.pad_images_and_timestamps_and_concat_poses(
                images,
                c2w_poses,
                intrinsics_fxfycxcy_px,
                selected_timesteps,
    ...
    def load_images_and_dims(self, image_paths, frame_indices):
        ...
        for image in images:
            assert image.shape[1] == orig_hw[0] and image.shape[2] == orig_hw[1], (
                f"shape {image.shape}"
            )
    ...
    def get_item(self, index, num_views, num_cond_views, aspect_ratio):
        ...
        while scene_scale_ratio > 1e4 or torch.isnan(scene_scale_ratio):
            ...
            if self.load_depths:
                depth_paths = self.get_depth_paths_for_seq(seq_name)
            else:
                depth_paths = None
            ...
            # Depth loading not available in external release
            world_points = None
            valid_depths = None
