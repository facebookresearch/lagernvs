...
def default_worker_init_fn(worker_id, num_workers, epoch, seed=0):
    ...
    gets a unique random seed for reproducibility. Also configures logging
    so that logger.error() calls in worker processes are visible.
    ...
    """
    # Configure logging in worker processes so logger.error() calls are visible
    # Without this, logger calls in __getitem__ would be silently dropped
    logging.basicConfig(
        level=logging.ERROR,
